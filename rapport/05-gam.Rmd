...
df_gam %>%
  ...
  labs(x = "", 
       y = "Prediction", 
       title = "Claim amount by interaction between marital status and gender")+
...
### Debiasing {.tabset .tabset-dropdown}
...
pred_full <- data.frame( prediction = sgl$predict(as_task_regr(test_new,target = "ClaimAmount"))$response)
...
ggplot(both)+
    geom_boxplot(mapping = aes(y=prediction, x=Gender,fill = model))+
    theme_minimal(base_size = 14) +
    theme(# Remove legend if color differentiation by Gender is not needed
        plot.title = element_text(face = "bold", size = 16),
        plot.subtitle = element_text(size = 12),
        axis.title.x = element_text(vjust = -0.2),
        axis.title.y = element_text(vjust = 1.2),
...
```{r, echo = FALSE, fig.height= 8, fig.width=8}
ggplot(both)+
    geom_boxplot(mapping = aes(y=prediction, x=MariStat,fill = model))+
    theme_minimal(base_size = 14) +
    theme(  # Remove legend if color differentiation by Gender is not needed
        plot.title = element_text(face = "bold", size = 16),
        plot.subtitle = element_text(size = 12),
...
###
When we looked at how our GAM implementation performed earlier, it appeared that GAM were fairly unbiased across the interactions between our protected features. But when we look at our plots with Marital Status, and our interaction between Marital Status and gender, that we were successful in creating a more unbiased predictions. But based on gender we can't clearly see that our debiased model is less biased than the biased GAM model. The reason why this is, is not clear, but a possible explanation could be because when debiasing we also take the average with respect to the marital status feature.
