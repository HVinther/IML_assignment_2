...
To inspect if a bias in the model exists with respect to the features gender and marital status, we plot the predictions of test set and stratify these by the interaction of these features. The plot can be seen below. Overall we see very little difference in the predictions between the factors. 
...
test %>% mutate(
  ...
  ggplot(aes(y = value, 
             x = interaction(Gender,MariStat),
             fill = interaction(Gender,MariStat)))+
  geom_boxplot()+
