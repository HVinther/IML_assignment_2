...
readRDS("shap/random_forest_explain")
...
shap_rf1<- plot(predict_parts(ranger_explain,dat_exp1[,-23]))
shap_rf2<- plot(predict_parts(ranger_explain,dat_exp2[,-23]))

predictor <- Predictor$new(ranger_explain,data=train[,-c(3,4,19)],y=train[,19])

future::plan(multisession)

importance<- FeatureImp$new(predictor,loss="mse",n.repetitions=15)
